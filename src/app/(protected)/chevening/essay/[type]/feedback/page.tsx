...
export default function FeedbackPage({ params }: { params: Promise<{ type: string }> }) {
  ...
  useSetPageMetadata({
    ...
    breadcrumbs: [
      ...
      { label: typeInfo ? t(`chevening.essayTypes.${typeInfo.enum}`) : "Essay", href: `/chevening/essay/${type}` },
      { label: t("feedback.feedback") },
    ],
  });

  function getApprovalInfo(prob: string) {
    switch (prob) {
  ...
  if (!typeInfo) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <Button variant="outline" className="mt-4" onClick={() => router.push("/chevening")}>
          {t("common.back")}
        </Button>
      </div>
    );
  ...
  if (!essay || !latestAnalysis || !feedback) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        ...
        <Button onClick={() => router.push(`/chevening/essay/${type}`)}>
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h2 className="text-lg font-bold text-foreground">{t(`chevening.essayTypes.${typeInfo.enum}`)}</h2>
      ...
      <div className="flex items-center gap-3 pb-4">
        <Button variant="outline" onClick={() => router.push(`/chevening/essay/${type}`)}>
          <PenLine className="h-4 w-4" />
