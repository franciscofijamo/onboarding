...
function normalizeBillingSource(value: unknown) {
  const normalized = typeof value === 'string' ? value.toLowerCase() : ''
  return normalized === 'clerk' ? 'clerk' : 'manual'
}
...
function validateAsaasMinimumValue(cents: number | null, currency: string): { valid: boolean; error?: string } {
  if (currency !== 'brl') {
    return { valid: true }
  }
  // Allow null (free plan) and 0 (free plan)
  if (cents === null || cents === 0) {
    return { valid: true }
  }

  // Asaas requires minimum R$ 5.00 (500 cents)
  const minCents = ASAAS_MIN_VALUE * 100
  if (cents < minCents) {
    return {
      valid: false,
...
async function handleAdminPlansGet() {
  ...
  return NextResponse.json({
    plans: plans.map(p => ({
      ...
      billingSource: p.billingSource ?? 'manual',
    })),
  })
}
...
async function handleAdminPlansPost(req: Request) {
  ...
  try {
    ...
    const yearlyPriceCents = toCents(body.priceYearlyCents)
    ...
    try {
      ...
      const data: Prisma.PlanCreateInput = {
        ...
        clerkName,
        currency,
        priceMonthlyCents: monthlyPriceCents,
        priceYearlyCents: yearlyPriceCents,
...
async function handleAdminPlansPut(_req: Request, ctx: { params: Promise<{ clerkId: string }> }) {
  ...
  try {
    ...
    // Validate Asaas minimum values on update
